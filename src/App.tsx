...
const App: React.FC = () => {
  ...
  const handleToggleEmployeeHidden = async (emp: Employee) => {
    ...
    try {
      ...
      setEmployees(prev => prev.map(x => x.id === emp.id ? { ...x, hidden: !emp.hidden } : x));
      showNotification(emp.hidden ? `${emp.name} is now visible` : `${emp.name} hidden from Employees page`, 'success');
    } catch (error) { showFirebaseError(error, 'Failed to update employee visibility'); }
  };
  ...
  const visibleEmployeesForPage = employees.filter(e => showHiddenEmployees || !e.hidden);
    ...
    <div className="min-h-screen bg-[#121212] text-white font-sans">
      ...
      <div className="ml-72 p-8">
            ...
            <div className="bg-[#1E1E1E] rounded-2xl border border-gray-800 overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-max">
              ...
              <tbody>{students.filter(s => s.status === 'ACTIVE' && (!studentClassFilter || s.class === studentClassFilter)).filter(s => s.name.toLowerCase().includes(searchTerm.toLowerCase()) || s.autoId.toLowerCase().includes(searchTerm.toLowerCase())).slice(0, showAllFeesByStudent ? undefined : 5).map(s => {
                ...
                return (<tr key={s.id} className="border-t border-gray-800 hover:bg-gray-800/30 transition">
                  ...
                  <td className="px-4 py-4"><span className="px-2 py-1 bg-purple-500/20 text-purple-400 rounded text-xs font-semibold">{s.class}</span></td>
        ...
        {activeTab === 'employees' && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-[#1E1E1E] p-4 rounded-xl border border-gray-800"><p className="text-gray-400 text-xs">Total Employees</p><p className="text-2xl font-bold text-cyan-400">{visibleEmployeesForPage.length}</p></div>
              <div className="bg-[#1E1E1E] p-4 rounded-xl border border-gray-800"><p className="text-gray-400 text-xs">Active</p><p className="text-2xl font-bold text-emerald-400">{visibleEmployeesForPage.filter(e => e.status === 'ACTIVE').length}</p></div>
              ...
              <div className="bg-[#1E1E1E] p-4 rounded-xl border border-gray-800"><p className="text-gray-400 text-xs">Total Paid</p><p className="text-2xl font-bold text-red-400">₹{visibleEmployeesForPage.reduce((s, e) => s + getEmployeeExpenseInfo(e).totalPaid, 0).toLocaleString()}</p></div>
            </div>
