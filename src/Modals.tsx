...
export const AppModals: React.FC<ModalProps> = (p) => {
  ...
  const renderExpenseForm = () => (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        {p.expenseForm.category === 'Salaries' && (
          <div className="space-y-1"><label className="text-xs text-cyan-400">Salary Month <span className="text-gray-500">(report uses this)</span></label><input type="month" value={p.expenseForm.salaryMonth || ''} onChange={e => p.setExpenseForm({ ...p.expenseForm, salaryMonth: e.target.value })} className={inputCls} /></div>
        )}
  ...
  const renderEmployeeForm = () => (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1"><label className="text-xs text-cyan-400">CL Quota / Year</label><input type="number" min={0} max={365} placeholder="Leave blank for global default" value={(p.employeeForm as any).clQuota || ''} onChange={e => p.setEmployeeForm(prev => ({ ...(prev as any), clQuota: parseFloat(e.target.value) || 0 }))} className={inputCls} /><p className="text-xs text-gray-500">Total CL days per year. Leave 0 or blank for global default.</p></div>
        <div className="space-y-1"><label className="text-xs text-cyan-400">CL Max / Month (Optional)</label><input type="number" min={0} max={31} placeholder="0 = unlimited carry-forward" value={(p.employeeForm as any).clAllowance || ''} onChange={e => p.setEmployeeForm(prev => ({ ...(prev as any), clAllowance: parseFloat(e.target.value) || 0 }))} className={inputCls} /><p className="text-xs text-gray-500">Optional hard cap per month. Applies only to current/future months — old entries are never affected. Leave 0 for unlimited carry-forward.</p></div>
        <div className="space-y-1"><label className="text-xs text-cyan-400">Other Deduction (₹) — Default</label><input type="number" min={0} placeholder="e.g. 500" value={(p.employeeForm as any).otherDeduction || ''} onChange={e => p.setEmployeeForm(prev => ({ ...(prev as any), otherDeduction: parseFloat(e.target.value) || 0 }))} className={inputCls} /><p className="text-xs text-gray-500">Default deduction every month (shown as Others(-) in salary slip). For month-specific amounts, use the Deduct button on the Employees page.</p></div>
  ...
  const renderOfferLetterSettings = () => {
    ...
    const isSavingOffer = !!p.settingsSaving;
    const handleSaveOffer = async () => {
      if (p.onSaveSchoolSettings) {
        const ok = await p.onSaveSchoolSettings();
        if (ok) p.setShowOfferLetterSettings(false);
      } else {
        p.setShowOfferLetterSettings(false);
      }
    };
    const addPoint = () => {
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-xs text-cyan-400 font-semibold">Points — Add Point + Descriptions <span className="text-gray-500 font-normal">(descriptions stay under same point via ::, not as separate points)</span></label>
            ...
            {pts.length === 0 && <p className="text-xs text-gray-500 italic">No points. Click "Add Point" — then add descriptions under that point.</p>}
            {pts.map((pt, i) => (
              <div key={i} className="flex gap-2 items-start bg-gray-800/50 rounded-lg p-3 border border-gray-700">
                <div className="flex-1 space-y-2">
                  <input value={pt.t} onChange={e => updateTitle(i, e.target.value)} className="w-full p-2 bg-gray-800 rounded-lg border border-gray-700 text-white text-sm font-semibold" placeholder="Point title (e.g., Appointment)" />
                  {pt.d.map((desc, di) => (
                    <div key={di} className="flex gap-1 items-center">
                      <span className="text-cyan-400 text-xs shrink-0">•</span>
                      <input value={desc} onChange={e => updateDesc(i, di, e.target.value)} className="flex-1 p-1.5 bg-gray-900 rounded-lg border border-gray-700 text-white text-xs" placeholder={`Description ${di + 1} for "${pt.t || 'Point ' + (i+1)}"`} />
                  ...
                  <button onClick={() => addDesc(i)} className="flex items-center gap-1 text-xs text-cyan-400 hover:text-cyan-300"><FiPlus size={12} /> Add description to this point</button>
                  <p className="text-[10px] text-gray-500">Stored as: <code className="bg-black/30 px-1 rounded">{pt.t || 'Title'} | {(pt.d.filter(Boolean).join(' :: ') || 'Desc1 :: Desc2')}</code> — one line = one numbered point, `::` bullets stay under it.</p>
                </div>
        ...
        <div className="flex gap-3">
          <button onClick={handleSaveOffer} disabled={isSavingOffer} className={`flex-1 p-3 rounded-lg font-bold flex items-center justify-center gap-2 ${isSavingOffer ? 'bg-gray-600 text-gray-300' : 'bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white'}`}>{isSavingOffer ? 'Saving…' : 'Save Offer Settings'}</button>
          <button onClick={() => p.setShowOfferLetterSettings(false)} className="px-4 py-3 bg-gray-800 hover:bg-gray-700 text-white rounded-lg">Cancel</button>
        </div>
        <p className="text-[11px] text-center text-gray-500">All Offer Letter settings save to <code className="bg-gray-800 px-1 rounded">localStorage</code> + <code className="bg-gray-800 px-1 rounded">Firebase settings/school</code> (via Save). `offerPoints` with `::` stays as bullets under same point.</p>
      </div>
    );
  };
