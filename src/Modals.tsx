  ...
  const renderEmployeeForm = () => (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1"><label className="text-xs text-cyan-400">CL Monthly Limit</label><input type="number" min={0} max={31} placeholder="e.g. 1" value={(p.employeeForm as any).clAllowance || ''} onChange={e => p.setEmployeeForm(prev => ({ ...(prev as any), clAllowance: parseFloat(e.target.value) || 0 }))} className={inputCls} /><p className="text-xs text-gray-500">Max CL days usable per month. Extra absences = salary deduction.</p></div>
